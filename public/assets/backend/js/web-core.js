            ...
            $.get(url, params, function (data, status) {
                ...
                if (data.next_page_url != null) {
                    AIZ.uploader.data.next_page_url =
                        AIZ.uploader.normalizePageUrl(data.next_page_url);
                ...
                if (data.prev_page_url != null) {
                    AIZ.uploader.data.prev_page_url =
                        AIZ.uploader.normalizePageUrl(data.prev_page_url);
    ...
    AIZ.extra = {
        ...
        addMore: function () {
            $('[data-toggle="add-more"]').each(function () {
                ...
                var target = $this.data("target");
                var countSelector = $this.data("count-selector") || ".remove-parent";

                // Allow limit override from URL for quick testing:
                // ?add_more_limit=unlimited|3|5|any
                var urlLimitRaw = null;
                try {
                    var searchParams = new URLSearchParams(window.location.search);
                    urlLimitRaw = searchParams.get("add_more_limit");
                } catch (e) {}

                var dataLimit = $this.data("limit");
                var dataMax = $this.data("max");
                var limitRaw =
                    urlLimitRaw !== null && urlLimitRaw !== ""
                        ? urlLimitRaw
                        : typeof dataLimit !== "undefined" && dataLimit !== ""
                          ? dataLimit
                          : dataMax;

                function parseAddMoreLimit(value) {
                ...
                var parsedLimit = parseAddMoreLimit(limitRaw);
                ...
                $this.on("click", function (e) {
                    ...
                    var currentCount = $target.find(countSelector).length;

                    // Block adding more when max limit is reached.
                    if (
                        parsedLimit !== null &&
                        parsedLimit !== Infinity &&
                        currentCount >= parsedLimit
                    ) {
                        alert(`Only ${parsedLimit} items can be added.`);
                        return;
                    }
        ...
        addMoreSortable: function (target, $trigger) {
            ...
            $target.find(itemsSelector).each(function () {
                ...
                if ($item.hasClass("remove-parent") && $item.find(".aiz-sortable-handle").length === 0) {
                    var $actionsCol = $item.find(".btn-dynamic-fields").first();
                    var handleHtml =
                        '<div class="aiz-sortable-handle btn btn-icon btn-circle btn-soft-info mb-1" style="cursor:move;">' +
                            '<i style="font-size: 16px;color: #889096;" class="ti ti-arrows-move"></i>' +
                        "</div>";

                    if ($actionsCol.length) {
                        $actionsCol.prepend(handleHtml);
                    } else {
                        $item.prepend(handleHtml);
                    }
                }
            });
