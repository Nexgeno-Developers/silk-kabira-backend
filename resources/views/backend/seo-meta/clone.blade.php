<form class="form" id="clone" action="{{ route($module . '.store') }}" method="POST">
    @csrf
    <div class="row">
        <div class="col-md-6">
            <div class="form-group mb-2">
                <label class="form-label">Slug <span class="text-danger">*</span></label>
                <input name="slug" type="text" class="form-control" maxlength="255" value="{{ $seoMeta->slug }}-copy" required>
            </div>
        </div>
...
$(document).ready(function() {
    ...
    $("#clone").submit(function(e) {
        var form = $(this);
        ajaxSubmit(e, form, callbackCloneForm);
    });

    const callbackCloneForm = function(response) {
        setTimeout(function() {
            location.reload();
        }, 1500);
    }
